...
(async () => {
    ...
    bot.on("messageCreate", async message => {
        ...
        if (!command)
            ...
            command.run({bot, message, args, config}); // 3 db? hmmm
    ...
    bot.on('interactionCreate', async interaction => {
        ...
        if (interaction.isButton()) {
            ...
            if (buttonID === ticketOpenChannel) { //ticket nyitas
                ...
                interaction.guild.channels.create(`ticket-${userticket.user.username}-${guildTickets.length + 1}`, {
            ...
            if (storedSettings > 0) {
                if (storedSettings.tickets.includes(buttonID)) { //ticket zárás gombbal
                ...
                if (storedSettings.tickets.includes(buttonID.replace("-save", ""))) { //ticket mentés
    ...
    bot.on('channelDelete', async (channel) => {
        ...
        if (!storedSettings) {
            ...
            storedSettings = await GuildSettings.findOne({ guildID: channel.guild.id });
        }
    ...
    bot.on("ready", async () => {
       ...
       // require("./saveData.js").init(log);
        ...
        console.log(`${bot.user.tag} elindult ${((new Date().getTime() - startTime) / 1000, 2).toFixed(2)}s alatt!`)
    })
    ...
    bot.on('guildMemberAdd', async(member) => {
        ...
        let roleid = storedSettings.role;
            ...
            if (storedSettings.autorole === false) {
                log(`Member has joined, but i don't have role id to add the role. Guild: "${guild}"`);
            } else {
